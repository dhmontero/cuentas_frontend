...
import com.cobiscorp.cobis.cntas.model.CuentaDestino;
...
@Properties({
  ...
  @Property(name = "bli.id", value = "BLI5933_bli_consultardestino")
})
public class BLI5933_bli_consultardestino implements IBLIExecutor {
  private static final ILogger logger = LogFactory.getLogger(BLI5933_bli_consultardestino.class);
  ...
  public void execute(DynamicRequest dynamicRequest) {
    ...
    try {
      ...
      if (dynamicRequest.getData().get("cuentaD") != null){
        mapper.addInputParameter("@i_cuenta", SqlType.VARCHAR, BLIUtils.convertToType(dynamicRequest.getData().get("cuentaD"), String.class));
      }
      ExecutorSP executorSP = new ExecutorSP(mapper);
      MapperResult mapperSp1 = new MapperResult();
      mapperSp1.addMapper(CuentaDestino.BANCO, 1);
      mapperSp1.addMapper(CuentaDestino.FECHACREACION, 2);
      mapperSp1.addMapper(CuentaDestino.FECHAMODIFICACION, 3);
      mapperSp1.addMapper(CuentaDestino.CLIENTE, 4);
      mapperSp1.addMapper(CuentaDestino.SALDO, 5);
      ...
      if (ret && (retCode == 0)) {
        if (mapper.getResults().size() >= 1) {
          ...
          dynamicRequest.setEntityList(CuentaDestino.ENTITY_NAME, del1);
        }
